...
- **🆕 v0.30.1 — Sandbox build directive**（**v0.41.0 修訂：先查 SDK MockProvider**）：

    **Step 0（v0.41.0 新增 — 強制第一步）**：main agent **必須**先讀 SDK README + 看 testing section 是否提供 mock implementation（如 `MockProvider` / `MockClient` / `FakeAdapter` 等）。**若有 → 跳過 sandbox 自蓋，用 SDK 內建 mock**：
    - SDK 內建 mock 通常 (a) 與真實 adapter 共用 Zod schema 驗證；(b) 內建狀態機；(c) capabilities 強制；(d) 失敗注入介面（如 `failNext()`）；(e) in-process 不需 docker
    - 自蓋 stub **永遠**比 SDK mock 信心差（會跟真實 adapter 漂移）
    - 教訓來源：[einvoice 案例 SEC-022](../../../examples/einvoice-n8n/SECURITY-REVIEW.md) — v0.30.1 implementing AI 沒讀完 SDK README，自蓋 5 個 vendor router，到 v0.40.0 真實 sandbox 測試時才發現 SDK `MockProvider` 一直都在且比手寫 stub 嚴格得多。
    ...
    **Critic gate（v0.41.0 強化）**：
    1. 若 main 開始建 sandbox vendor router 但 commit message / artefact 內**沒有「已查 SDK README 確認無 MockProvider」evidence** → **VETO**（lexical regex 偵測 `MockProvider` / `mock provider` / `failNext` / `stub` 字眼於 Stage 8 commit 範圍）
