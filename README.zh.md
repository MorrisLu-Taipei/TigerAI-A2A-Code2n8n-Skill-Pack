...
> 📊 **一張圖看懂**：自然語言需求或既有程式系統 → Code2n8n Skill Pack（Cookbook + 2,061 參考 workflow *為設計查找語料，非已驗證模板* + DSL v1.2 + 14 個 Skill + 4 大企業模式）→ 分析哪些邏輯留在程式、哪些上升為 n8n 節點 → 產出可檢查、可交接、可跨系統編排的 workflow。
...
| [台灣電子發票統一 SDK](examples/einvoice-n8n/) | TypeScript SDK（[`MorrisLu-Taipei/einvoice`](https://github.com/MorrisLu-Taipei/einvoice)，5 家供應商）→ 80 行 Hono wrapper svc + 6 個治理 workflow | 靜態 lint 0 err / 3 expected webhook warn · n8n REST import 6/6 在 localhost:5678 跑通 · 組合 v0.26.0 三個模板（`retry-with-backoff` + `human-approval-gate` + `handover-trace`） |
...
│   └── 00-INDEX.md
│
├── skills/                    ← 14 個 Skill（plugin.json manifest 與磁碟一致）
